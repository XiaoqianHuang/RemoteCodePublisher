...
namespace Convert
{
	...
	public:
		...
		void InsertWalk(CodeAnalysis::ASTNode* pItem, size_t& count);
	...
	inline std::string Display::insertClassAndFunction(const std::string& temp, CodeAnalysis::ASTNode* pScope) {
		...
		InsertWalk(pScope, initialcount);
		insertedtext = InsertTheTag(insertedtext);
	...
	inline void Display::InsertWalk(CodeAnalysis::ASTNode* pItem, size_t& count)
	{
		size_t inCount = ++count;
		auto iter = pItem->children_.begin();
		while (iter != pItem->children_.end())
		{
			InsertWalk(*iter, count);
			++iter;
		} 
		Info newinfo;
		newinfo.startline = pItem->startLineCount_;
		newinfo.endline = pItem->endLineCount_;
		newinfo.type = pItem->type_;
		typeInfoTable.push_back(newinfo);
	}
	...
	std::string Display::InsertTheTag(const std::string& temp) {
