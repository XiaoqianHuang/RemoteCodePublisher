...
namespace Utilities
{
  ...
  private:
	  ...
	  bool isAppPath(const std::string& path);
  private:
    Usage usage_;
    Path path_;
    Patterns patterns_;
  ...
  inline std::string ProcessCmdLine::appPath()
  {
	  return appPath_;
  }
  ...
  inline void ProcessCmdLine::setAppPath(const std::string& appPath)
  {
	  appPath_ = appPath;
  }
  ...
  /*----< judge if is appliaction path >-------------------------------------*/

  inline bool ProcessCmdLine::isAppPath(const std::string& path)
  {
	  bool isp = 0;
	  size_t pos = 0;
	  pos = path.find("firefox.exe");
  ...
  inline ProcessCmdLine::ProcessCmdLine(int argc, char** argv, std::ostream& out) : out_(out)
  {
    ...
    for (int i = 1; i < argc; ++i)
    {
      ...
      else
      {
		...
		else if (isAppPath(argv[i]))// if the argument is browser path
		{
			appPath_ = argv[i];
		}
